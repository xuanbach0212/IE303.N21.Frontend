...
const AdminMessages = () => {
    ...
    useEffect(() => {
        const fetchUserMessages = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://54.179.229.192:8080/api/messages/search/findByClosed/?closed=false&page=${
                    currentPage - 1
    ...
    const submitResponseToQuestion = async (id: number, response: string) => {
        const url = `http://54.179.229.192:8080/api/messages/secure/admin/message`;

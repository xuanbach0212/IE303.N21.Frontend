...
const BookCheckoutPage = () => {
    ...
    useEffect(() => {
        const fetchBook = async () => {
            const baseUrl: string = `http://54.255.135.69:8080/api/books/${bookId}`;
    ...
    useEffect(() => {
        const fetchBookReviews = async () => {
            const reviewUrl: string = `http://54.255.135.69:8080/api/reviews/search/findByBookId?bookId=${bookId}`;
    ...
    useEffect(() => {
        const fetchUserReviewBook = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://54.255.135.69:8080/api/reviews/secure/user/book/?bookId=${bookId}`;
    ...
    useEffect(() => {
        const fetchUserCurrentLoansCount = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://54.255.135.69:8080/api/books/secure/currentloans/count`;
                const requestOptions = {
                    method: 'GET',
                    headers: {
                        Authorization: `Bearer ${authState.accessToken?.accessToken}`,
    ...
    useEffect(() => {
        const fetchUserCheckedOutBook = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://54.255.135.69:8080/api/books/secure/ischeckedout/byuser/?bookId=${bookId}`;
                const requestOptions = {
                    method: 'GET',
                    headers: {
                        Authorization: `Bearer ${authState.accessToken?.accessToken}`,
    ...
    const checkoutBook = async () => {
        const url = `http://54.255.135.69:8080/api/books/secure/checkout/?bookId=${book?.id}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    const submitReview = async (starInput: number, reviewDescription: string) => {
        ...
        const url = `http://54.255.135.69:8080/api/reviews/secure`;
        const requestOptions = {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
